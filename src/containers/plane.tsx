...
import { Deploy as DeployAPI, Mod as ModAPI } from "../api"
...
export function Plane() {
  ...
  async function loadData() {
    const data = await DeployAPI.read()
    data.sort((a, b) => {
  ...
  return (
    <div className="plane-box">
      {deploy?.map(function (item) {
        return <PlaneDeploy key={item.id} deploy={item} onDelete={onDelete}></PlaneDeploy>
      })}
    </div>
  )
}
...
function ButtonBox(props: ButtonBoxProps) {
  ...
  return (
    <div className="buttons-box">
      ...
      <Button tip="保存部署" load={states.deploying} load_tip="部署中" onClick={clickSave}>
