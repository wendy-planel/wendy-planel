...
import { ModConfig, Deploy as DeploySchema, PublishedFileDetail } from '../common/interface'
import {
  LoadIcon,
  StopIcon,
  CopyIcon,
...
function Mod(props: ModProps) {
  ...
  const onAdd = async () => {
    ...
    if (config.length > 0) {
      ...
      mod.code = mod_config.code
      const lua = await factory.createEngine()
      await lua.doString(mod_config.code)
      mod.configuration_options = lua.global.get('configuration_options')
      pick.push(mod)
      setContent(produce(draft => {
        draft.pick = pick
        draft.search = search
  ...
  return (
    <div className="mod-item">
      ...
      <div className="mod-item-left">
        ...
        {
          mode === 'add' ? <div className="mod-item-button">
            <div onClick={onAdd}>

              {!adding ? <HoverTip tip="点击添加"><svg
                viewBox="0 0 1024 1024"
                width="18"
                height="18"
                fill="#3498db"
              ...
              >
                ...
                <path d="M675.413333 546.133333h-307.2c-18.773333 0-34.133333-15.36-34.133333-34.133333s15.36-34.133333 34.133333-34.133333h307.2c18.773333 0 34.133333 15.36 34.133334 34.133333s-14.933333 34.133333-34.133334 34.133333z" fill="#4C4C4C" p-id="13993"></path><path d="M487.68 665.6V358.4c0-18.773333 15.36-34.133333 34.133333-34.133333s34.133333 15.36 34.133334 34.133333v307.2c0 18.773333-15.36 34.133333-34.133334 34.133333s-34.133333-15.36-34.133333-34.133333z"></path>
              </svg> </HoverTip> : <HoverTip tip="添加模组中"><svg
                fill="#3498db"
                viewBox="0 0 1024 1024"
                width="18"
                height="18"
  ...
  async function handleSearch() {
    if (key === '') {
      ...
      setContent(produce(draft => {
        ...
        draft.search = search.filter(item => !mods.includes(item.publishedfileid))
      }))
    }
  }
  return (
    <>
      ...
      {
        content.state === 'searching' && <div className="mod-box-loading">
          <HoverTip tip='搜索中'>
            <svg
              fill="#3498db"
              viewBox="0 0 1024 1024"
      ...
      {
        content.search.length > 0 && content.state !== 'searching' && <div className="mod-box-search">
          {
...
function ModBox(props: ModBoxProps) {
  const { deploy } = props
  const [content, setContent] = useState<ModBoxContent>({ pick: [], search: [] })
  async function loadData() {
    const mods: string[] = []
  ...
  return (
    <div className="mod-box">
      ...
      <div className="mod-box-added" style={{ height: content.search.length <= 0 && content.state !== 'searching' ? '78%' : '45%' }}>
        {
          content.state === 'parsing' ? <div className="mod-box-loading">
